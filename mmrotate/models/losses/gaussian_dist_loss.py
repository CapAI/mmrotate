...
def kld_loss(pred, target, fun="log1p", tau=1.0, alpha=1.0, sqrt=True):
    ...
    with torch.autograd.set_detect_anomaly(True):
        ...
        Sigma_p_inv = Sigma_p_inv / Sigma_p.det().clamp(1e-7).unsqueeze(-1).unsqueeze(-1)
        ...
        distance = postprocess(distance, fun=fun, tau=tau)

        if distance.isnan().any() or Sigma_p_inv.isnan().any() or Sigma_p.isnan().any() or Sigma_t.isnan().any():
            print("nans!")
        return distance
...
class GDLoss(nn.Module):
    ...
    BAG_PREP = {"xy_stddev_pearson": xy_stddev_pearson_2_xy_sigma, "xy_wh_r": xy_wh_r_2_xy_sigma}
