...
from mmdet.utils import ConfigType, InstanceList, OptConfigType, OptInstanceList, reduce_mean
...
class RotatedFCOSHead(FCOSHead):
    ...
    def __init__(
        ...
        h_bbox_coder: ConfigType = dict(type="mmdet.DistancePointBBoxCoder"),
        ...
        loss_bbox: ConfigType = dict(type="RotatedIoULoss", loss_weight=1.0),
    ...
    def loss_by_feat(
        ...
        labels, bbox_targets, angle_targets = self.get_targets(all_level_points, batch_gt_instances)
        ...
        flatten_angle_preds = [angle_pred.permute(0, 2, 3, 1).reshape(-1, angle_dim) for angle_pred in angle_preds]
        ...
        with torch.cuda.amp.autocast(enabled=False):
            loss_cls = self.loss_cls(flatten_cls_scores.float(), flatten_labels, avg_factor=num_pos)
        ...
        if len(pos_inds) > 0:
            ...
            if self.use_hbbox_loss:
                ...
                pos_bbox_preds = torch.cat([pos_bbox_preds, pos_decoded_angle_preds], dim=-1)
            ...
            with torch.cuda.amp.autocast(enabled=False):
                loss_bbox = self.loss_bbox(
                    pos_decoded_bbox_preds.float(),
                    pos_decoded_target_preds.float(),
        ...
        if self.loss_angle:
            return dict(loss_cls=loss_cls, loss_bbox=loss_bbox, loss_angle=loss_angle, loss_centerness=loss_centerness)
        else:
    ...
    def _get_targets_single(
        self, gt_instances: InstanceData, points: Tensor, regress_ranges: Tensor, num_points_per_lvl: List[int]
    ...
    def _predict_by_feat_single(
        ...
        for level_idx, (cls_score, bbox_pred, angle_pred, score_factor, priors) in enumerate(
            zip(cls_score_list, bbox_pred_list, angle_pred_list, score_factor_list, mlvl_priors)
        ):
            ...
            results = filter_scores_and_topk(
                scores, score_thr, nms_pre, dict(bbox_pred=bbox_pred, angle_pred=angle_pred, priors=priors)
            )
