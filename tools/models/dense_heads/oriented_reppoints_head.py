...
from mmrotate.models.dense_heads.rotated_reppoints_head import RotatedRepPointsHead
from mmrotate.registry import MODELS
from ..utils import levels_to_images


def ChamferDistance2D(point_set_1: Tensor, point_set_2: Tensor, distance_weight: float = 0.05, eps: float = 1e-12):
...
class OrientedRepPointsHead(RotatedRepPointsHead):
    ...
    def __init__(
        self,
        *args,
        loss_spatial_init: ConfigType = dict(type="SpatialBorderLoss", loss_weight=0.05),
        loss_spatial_refine: ConfigType = dict(type="SpatialBorderLoss", loss_weight=0.1),
        top_ratio: float = 0.4,
    ...
    def _get_targets_single(
        ...
        if not inside_flags.any():
            raise ValueError("There is no valid proposal inside the image boundary. Please " "check the image size.")
        ...
        if unmap_outputs:
            ...
            labels = unmap(labels, num_total_proposals, inside_flags, fill=self.num_classes)  # fill bg label
    ...
    def loss_by_feat_single(
        ...
        loss_cls = self.loss_cls(cls_score, labels, label_weights, avg_factor=avg_factor_refine)
    ...
    def loss_by_feat(
        ...
        center_list, valid_flag_list = self.get_points(featmap_sizes, batch_img_metas, device)
        pts_coordinate_preds_init = self.offset_to_pts(center_list, pts_preds_init)

        num_proposals_each_level = [(featmap.size(-1) * featmap.size(-2)) for featmap in cls_scores]

        num_level = len(featmap_sizes)
        assert num_level == len(pts_coordinate_preds_init)

        if "ConvexAssigner" in self.train_cfg.init.assigner["type"]:
        ...
        (*_, bbox_gt_list_init, candidate_list_init, bbox_weights_list_init, num_total_pos_init, num_total_neg_init) = (
        ...
        (refine_points_features,) = multi_apply(
            self.get_adaptive_points_feature, base_feat, pts_coordinate_preds_refine, self.point_strides
        )
        features_pts_refine = levels_to_images(refine_points_features)
        features_pts_refine = [item.reshape(-1, self.num_points, item.shape[-1]) for item in features_pts_refine]
        ...
        for i_img, center in enumerate(center_list):
            ...
            for i_lvl in range(len(pts_preds_refine)):
                ...
                points_preds_init_ = points_preds_init_.view(
                    points_preds_init_.shape[0], -1, *points_preds_init_.shape[2:]
                )
        ...
        pts_coordinate_preds_init_img = levels_to_images(pts_coordinate_preds_init, flatten=True)
        pts_coordinate_preds_init_img = [
            item.reshape(-1, 2 * self.num_points) for item in pts_coordinate_preds_init_img
        ]

        pts_coordinate_preds_refine_img = levels_to_images(pts_coordinate_preds_refine, flatten=True)
        pts_coordinate_preds_refine_img = [
            item.reshape(-1, 2 * self.num_points) for item in pts_coordinate_preds_refine_img
        ]
        ...
        with torch.no_grad():
            ...
            labels_list, label_weights_list, bbox_weights_list_refine, num_pos, pos_normalize_term = multi_apply(
        ...
        bbox_gt_refine = torch.cat(bbox_gt_list_refine, 0).view(-1, bbox_gt_list_refine[0].size(-1))
        ...
        pos_inds_flatten = ((0 <= labels) & (labels < self.num_classes)).nonzero(as_tuple=False).reshape(-1)

        assert len(pos_normalize_term) == len(pos_inds_flatten)

        if num_pos:
            losses_cls = self.loss_cls(cls_scores, labels, labels_weight, avg_factor=num_pos)
    ...
    def sampling_points(self, polygons: Tensor, points_num: int, device: str) -> Tensor:
        ...
        for i in range(4):
            if i < 3:
                points_x = ratio * polygons_xs[:, i + 1 : i + 2] + (1 - ratio) * polygons_xs[:, i : i + 1]
                points_y = ratio * polygons_ys[:, i + 1 : i + 2] + (1 - ratio) * polygons_ys[:, i : i + 1]
            else:
                points_x = ratio * polygons_xs[:, 0].unsqueeze(1) + (1 - ratio) * polygons_xs[:, i].unsqueeze(1)
                points_y = ratio * polygons_ys[:, 0].unsqueeze(1) + (1 - ratio) * polygons_ys[:, i].unsqueeze(1)
    ...
    def get_adaptive_points_feature(self, features: Tensor, pt_locations: Tensor, stride: int) -> Tensor:
        ...
        pt_locations[..., 1] = pt_locations[..., 1] / (h / 2.0) - 1

        batch_size = features.size(0)
        sampled_features = torch.zeros(
            [pt_locations.shape[0], features.size(1), pt_locations.size(1), pt_locations.size(2)]
    ...
    def feature_cosine_similarity(self, points_features: Tensor) -> Tensor:
        ...
        feats_similarity = 1.0 - F.cosine_similarity(unity_points_features, unity_mean_points_feats, dim=2, eps=1e-6)

        max_correlation, _ = torch.max(feats_similarity, dim=1)

        return max_correlation

    def pointsets_quality_assessment(
        self,
        pts_features: Tensor,
        cls_score: Tensor,
        pts_pred_init: Tensor,
        ...
        qua_cls = self.loss_cls(
            pos_scores, pos_label, pos_label_weight, avg_factor=self.loss_cls.loss_weight, reduction_override="none"
        )
        ...
        qua = (
            qua_cls
            + self.init_qua_weight * (qua_loc_init + qua_ori_init)
            + (1.0 - self.init_qua_weight) * (qua_loc_refine + qua_ori_refine)
            + qua_poc
        )
    ...
    def dynamic_pointset_samples_selection(
        ...
        if len(pos_inds) == 0:
            return label, label_weight, bbox_weight, 0, Tensor([]).type_as(bbox_weight)
        ...
        for gt_ind in range(num_gt):
            ...
            for level in range(num_level):
                ...
                value, topk_inds = quality[level_gt_mask].topk(min(level_gt_mask.sum(), 6), largest=False)
        ...
        pos_normalize_term = pos_level_mask_after_select * (
            self.point_base_scale * torch.as_tensor(self.point_strides).type_as(label)
        ).reshape(-1, 1)
    ...
    def init_loss_single(
        self, pts_pred_init: Tensor, bbox_gt_init: Tensor, bbox_weights_init: Tensor, stride: int
    ) -> Tuple[Tensor, Tensor]:
        ...
        loss_pts_init = self.loss_bbox_init(
            pts_pred_init_norm / normalize_term, bbox_gt_init_norm / normalize_term, bbox_weights_pos_init
        )
