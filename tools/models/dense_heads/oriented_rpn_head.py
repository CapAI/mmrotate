...
from mmdet.structures.bbox import BaseBoxes, get_box_tensor, get_box_wh, scale_boxes
...
class OrientedRPNHead(RPNHead):
    ...
    def _bbox_post_process(
        ...
        if results.bboxes.numel() > 0:
            ...
            # nms can only handle float32, not float16.
            dtype = hbboxes.dtype
            det_bboxes, keep_idxs = batched_nms(hbboxes.float(), results.scores.float(), results.level_ids, cfg.nms)
            det_bboxes = det_bboxes.to(dtype)

            results = results[keep_idxs.long()]
