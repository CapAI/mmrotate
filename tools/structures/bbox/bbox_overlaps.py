...
def rbbox_overlaps(bboxes1: Tensor, bboxes2: Tensor, mode: str = "iou", is_aligned: bool = False) -> Tensor:
    ...
    clamped_bboxes1 = bboxes1.detach().clone().float()
    clamped_bboxes2 = bboxes2.detach().clone().float()
    ...
    # TODO: fix in mmcv
    clamped_bboxes1[:, :2].clamp_(min=-1e6, max=1e7)
    clamped_bboxes2[:, :2].clamp_(min=-1e6, max=1e7)

    return box_iou_rotated(clamped_bboxes1, clamped_bboxes2, mode, is_aligned)


def fake_rbbox_overlaps(
    bboxes1: RotatedBoxes, bboxes2: RotatedBoxes, mode: str = "iou", is_aligned: bool = False
) -> Tensor:
