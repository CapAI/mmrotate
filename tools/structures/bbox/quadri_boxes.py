...
class QuadriBoxes(BaseBoxes):
    ...
    def areas(self) -> Tensor:
        ...
        xyxy = torch.sum(pts[..., 0] * roll_pts[..., 1] - roll_pts[..., 0] * pts[..., 1], dim=-1)
    ...
    def widths(self) -> Tensor:
        ...
        """
        warnings.warn(
            "Quadrilateral boxes don't have the width concept. " "We use ``sqrt(areas)`` to replace the width."
        )
    ...
    def heights(self) -> Tensor:
        ...
        """
        warnings.warn(
            "Quadrilateral boxes don't have the height concept. " "We use ``sqrt(areas)`` to replace the width."
        )
        return torch.sqrt(self.areas)
    ...
    def resize_(self, scale_factor: Tuple[float, float]) -> None:
        ...
        boxes = self.tensor
        assert len(scale_factor) == 2
        assert scale_factor[0] == scale_factor[1], "To protect the shape of QuadriBoxes not changes"
    ...
    def is_inside(self, img_shape: Tuple[int, int], all_inside: bool = False, allowed_border: int = 0) -> BoolTensor:
        ...
        return (
            (centers[..., 0] <= img_w + allowed_border)
